...
mongoose.connect(MONGODB_URL,)
    ...
    app.post('/update-telegram-Mushroom', async (req, res) => {
        const { telegramId } = req.body;
    
        try {
            const user = await UserProgress.findOne({ telegramId });
    
            if (user && !user.isSubscribedToMushroom) {
                user.coins += 250; // Начисляем 200 монет за подписку на Twitter
                user.isSubscribedToMushroom = true; // Помечаем, что пользователь подписан на Twitter
                await user.save(); // Сохраняем изменения в базе данных
            }
    
            return res.json({ success: true, isSubscribedToMushroom: user.isSubscribedToMushroom, coins: user.coins });
    ...
    app.post('/update-telegram-fuccdafomo', async (req, res) => {
        const { telegramId } = req.body;
    
        try {
            const user = await UserProgress.findOne({ telegramId });
    
            if (user && !user.isSubscribedTofuccdafomo) {
                user.coins += 500; // Начисляем 200 монет за подписку на Twitter
                user.isSubscribedTofuccdafomo = true; // Помечаем, что пользователь подписан на Twitter
                await user.save(); // Сохраняем изменения в базе данных
            }
    ...
    app.post('/update-telegram-Captcha', async (req, res) => {
        const { telegramId } = req.body;
    
        try {
            const user = await UserProgress.findOne({ telegramId });
    
            if (user && !user.isSubscribedToCaptcha) {
                user.coins += 500; // Начисляем 200 монет за подписку на Twitter
                user.isSubscribedToCaptcha = true; // Помечаем, что пользователь подписан на Twitter
                await user.save(); // Сохраняем изменения в базе данных
            }
    
            return res.json({ success: true,  isSubscribedToCaptcha: user. isSubscribedToCaptcha, coins: user.coins });
